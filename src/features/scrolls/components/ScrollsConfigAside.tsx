    ...
    <aside className="w-full lg:w-64 shrink-0 panel rounded p-3 flex flex-col gap-0 overflow-y-auto">
        ...
        {phasesWithMultiPnj.length > 0 && (
            <section className="mt-3 pt-3 border-t border-dofus-border/30">
                <p className="text-xs text-dofus-text-lt uppercase tracking-wider font-semibold mb-2">PNJ</p>
        ...
        <section className="mt-3 pt-3 border-t border-dofus-border/30 flex-1">
            <p className="text-xs text-dofus-text-lt uppercase tracking-wider font-semibold mb-2">Parchemins à utiliser</p>
            {phases.length === 0 ? (
                ...
                <table className="w-full text-xs border-collapse">
                        ...
                        {phases.map(phase => (
                            <tr key={phase.tierType} className="border-b border-dofus-border/20 last:border-0">
                                <td className="py-1.5 pr-2">
                                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium border ${TIER_COLORS[phase.tierType]}`}>
                                        {TIER_LABELS[phase.tierType]}
                                    </span>
                                </td>
                                <td className="py-1.5 font-bit text-dofus-orange font-mono text-right">
                                    {phase.scrollsNeeded}×
                                </td>
                                <td className="py-1.5 pl-2 text-right font-bit font-mono font-semibold text-dofus-gold">
                    ...
                    <tfoot>
                        <tr className="border-t-2 border-dofus-border/40">
                            ...
                            <td className="pt-2 text-right font-bit font-mono text-dofus-orange">{totalScrolls}</td>
                        </tr>
                    </tfoot>
                </table>
            )}
