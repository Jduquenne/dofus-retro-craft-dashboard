...
const LEGEND = [
  { color: '#2860A0', label: 'Zone bleue' },
  { color: '#A02828', label: 'Zone rouge' },
  { color: '#8B5C30', label: 'Obstacle' },
  { color: '#000000', label: 'Vide' },
] as const;
...
export function MapCellInfoPanel({ current, blueCellCount, redCellCount, obstacleCount, index, total, onPrev, onNext }: MapCellInfoPanelProps) {
  const subarea = current?.subarea;
  const subareaName = subarea?.name?.replace('//', '').trim();

  return (
    <div className="hidden sm:flex flex-col gap-3 w-44 shrink-0 bg-dofus-panel-dk/30 rounded-lg p-3">
      ...
      {/* Sélecteur de configuration (multi-maps) */}
      {total > 1 && (
        <>
          <div className="bg-dofus-border/20 h-px" />
          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] text-dofus-text-lt uppercase tracking-wider">Configuration</span>
      ...
      <div className="mt-auto flex flex-col gap-1.5">
        <div className="bg-dofus-border/20 h-px mb-1" />
        <span className="text-[9px] text-dofus-text-lt uppercase tracking-wider">Légende</span>
        {LEGEND.map(({ color, label }) => (
          <div key={label} className="flex items-center gap-2">
            <div className="w-2.5 h-2.5 rounded-sm shrink-0" style={{ background: color }} />
            <span className="text-[10px] text-dofus-text-lt">{label}</span>
          </div>
        ))}
      </div>
